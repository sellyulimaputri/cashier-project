    ...
    <table class="table">
        <thead>
            <tr>
                ...
                <th scope="col">Telepon Petugas</th>
            </tr>
        </thead>
        <tbody>
            ...
            @foreach ($petugas as $p)
                @if ($p->IsDelete == 0)
                    <tr>
                        <th scope="row">{{ $p->idPetugas }}</th>
                        <td>{{ $p->namaPetugas }}</td>
                        <td>{{ $p->alamatPetugas }}</td>
                        <td>{{ $p->teleponPetugas }}</td>
                    </tr>
                @endif
            @endforeach
        </tbody>
    ...
    <table class="table">
        ...
        <tbody>
            ...
            @foreach ($pelanggan as $p)
                @if ($p->IsDelete == 0)
                    <tr>
                        <th scope="row">{{ $p->idPelanggan }}</th>
                        <td>{{ $p->namaPelanggan }}</td>
                        <td>{{ $p->alamatPelanggan }}</td>
                        <td>{{ $p->teleponPelanggan }}</td>
                    </tr>
                @endif
            @endforeach
        </tbody>
...
@section('section')
    ...
    <script>
        $(document).ready(function() {
            $("#searchInputPelanggan").on("keyup", function() {
                searchTable();
            });
            $("#searchByPelanggan").change(function() {
                searchTable();
            });

            function searchTable() {
                var input, filter, table, tr, td, i, txtValue, searchBy;
                input = $("#searchInputPelanggan").val().toUpperCase();
                table = $(".table");
                tr = table.find("tr");
                searchBy = $("#searchByPelanggan").val();
                for (i = 0; i < tr.length; i++) {
                    td = tr[i].getElementsByTagName("td")[0]; // Ganti dengan indeks kolom yang ingin dicari
                    if (td) {
                        txtValue = td.textContent || td.innerText;
        ...
        <div class="col-12">
            <div class="card mb-4">
                <div class="card-header pb-0" style="display: flex; justify-content: space-between; align-items: center;">
                    <h6>Daftar Ketersediaan Produk</h6>
                    <div class="d-flex">
                        <select class="form-select me-2" id="searchByPelanggan">
                            <option value="name" class="">Nama</option>
                            <option value="category">Kategori</option>
                        </select>
